...
const ArticleList = () => {
    ...
    useEffect(() => {
        const fetchArticles = async () => {
            try {
                const response = await fetch("http://localhost/esercizio/wp-json/wp/v2/posts?_embed=1");
                if (!response.ok) {
              ...
              const deletePost = async (postId) => {
                  try {
     const authoString= "riccardo:MEfL 2NFv bMYe FeV0 At9U xLzH"
      
      const encodedAuthString = btoa(authoString);
      const response = await fetch(`http://localhost/esercizio/wp-json/wp/v2/posts/${postId}`, {
        method: "DELETE",
        headers: {
  ...
  const handleAddArticle = async () => {
    try {
      ...
      const encodedAuthString = btoa(authoString);
      const apiUrl = "http://localhost/esercizio/wp-json/wp/v2/posts";
  ...
  const handleEditArticle = async () => {
    try {
      ...
      const encodedAuthString = btoa(authoString);
      const apiUrl = `http://localhost/esercizio/wp-json/wp/v2/posts/${selectedArticle.id}`;
      const response = await fetch(apiUrl, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <>
        ...
        <h1 className="mt-5 mb-4"> ANIMUS article </h1>
              ...
              <div className="card">
                <img
                  src={
                    ...
                    : " https://c.wallhere.com/photos/9c/e9/logo_abstergo_Assassin's_Creed-1460365.jpg!d "
                  }
                ...
                <div className="card-footer" style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <div>
                    <button className="btn btn-primary me-2" onClick={() => openModal(article)}>
                      Visualizza
                    </button>
                    <button
